...
class TinyDB(private val appContext: Context) {
    ...
    fun getListInt(key: String): ArrayList<Int> {
        ...
        val arrayToList = ArrayList<String>(myList.asList())
    ...
    fun getListObject(key: String): ArrayList<BestPets> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList: ArrayList<BestPets> = ArrayList()
        for (jObjString in objStrings) {
            val player = gson.fromJson(jObjString, BestPets::class.java)
    ...
    fun <T> getObject(key: String, classOfT: Class<T>): T {
        ...
        val value: Any? = Gson().fromJson(json, classOfT)
        return value as T
    }
    ...
    fun putListObject(key: String, playerList: ArrayList<BestPets>) {
        checkForNullKey(key)
        val gson = Gson()
